...
class SingularLoRALinearLayer(nn.Module):
    ...
    def get_lambda(self, timestep):
        self.content = self.weight_1_a @ self.weight_1_b
        self.style = self.weight_2_a @ self.weight_2_b 
        U, S, Vh = torch.svd(W)
        #factor 만들기
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.forward_type == "merge":
            self.lambda = self.get_lambda(glo_count)
            glo_count += 1
